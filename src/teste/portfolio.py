...
def process_stock(frame):
    ...
    column_to_drop = ["fech_historico", "fator","tipo","vol_em_aluguel(mm_r$)","quant_em_aluguel"]
    frame = frame.drop(column_to_drop, axis=1)
    # Renaming to Close and Returns:
    frame = frame.rename(columns={"fech_ajustado": "Close", "variacao(pct)": "Returns","vol_(mm_r$)":"Volume"})
    # dropna:
    frame = frame.dropna()
    return frame
...
def split_data(data, rate=0.8):
    ...
    x = data.drop(["Close","return_signal","Returns"], axis=1)
    y = data.loc[:, "return_signal"]
    ...
    y_train = y[:split]
    y_test = y[split:]
    return_test = data["Returns"][split:]
    close_test = data["Close"][split:]
    return x_train, x_test, y_train, y_test, close_test, return_test
